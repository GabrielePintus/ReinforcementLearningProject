...
class TilingApproximator:
    ...
    def __init__(self, bounds, n_tiles, n_tilings, shifts):
        ...
        self.model = KernelRidge(kernel='rbf', gamma=0.5)
        self.model = KernelRidge(kernel='poly', degree=3)
...
if __name__ == "__main__":
    ...
    # f = lambda x: np.sum(x) + np.random.normal(0, 0.5)
    # f = lambda x: np.sin(x[0]) + np.cos(x[1]) + np.random.normal(0, 0.1)
    f = lambda x: np.sin(x[0]) + np.cos(x[1]) * x[0] + np.random.normal(0, 0.1)
    ...
    tiling = TilingApproximator(bounds, n_tiles, n_tilings, shifts)
    
    tiling.fit(X, y)

    y_pred = np.array([tiling.predict(x) for x in X]).round(2)

    for i in range(len(X)):
        print(f"prediction: {y_pred[i]}, true: {y[i]}")
    ...
    ax.scatter(X[:, 0], X[:, 1], y, color='r', marker='o', label='True')
    ax.scatter(X[:, 0], X[:, 1], y_pred, color='b', marker='x', label='Predicted')
